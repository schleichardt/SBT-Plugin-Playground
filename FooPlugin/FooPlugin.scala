...
object FooPlugin extends Plugin {
  //see http://harrah.github.com/xsbt/latest/api/#sbt.Plugin
  ...
  lazy val helloCommand =
    Command.command("hello", "Simple Hello World Command", "detailed description of hello") {
      (state: State) =>
        println("Hi!  ")
        state
    }
  ...
  lazy val helloCommandWithArgsFromProject =
    Command.command("hello2", "bla 2", "blubb 2") {
      (state: State) =>
  ...
  lazy val callOtherCommandFromSbt = Command.command("hello7", "bla 7", "blubb 7") {
    (state: State) =>
  ...
  lazy val callOtherTaskFromPlugin = Command.command("hello8", "bla 8", "blubb 8") {
    (state: State) =>
  ...
  val newSettings = Seq(
    ...
    newTask2 <<= (newSetting, newSetting2) map {
      (set1, set2) => println(set1 + " " + set2)
    }, //vor map als tuple, und als anonyme Funktion auch wie Tuple aufgebaut
    newTask3 <<= name map {
      x => println("TODO" + x)
